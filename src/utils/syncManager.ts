...
export async function handleBidirectionalSync(
  ...
  if (cleanPush) {
    console.log(
      'Clean push enabled – pushing ALL keys from languageData.json to the spreadsheet...',
    );
  ...
  console.log(
    `Found local changes (${pushMode}): ${localesCount} locale(s), ~${keysCount} key(s) to sync to the spreadsheet.`,
  );
