...
export async function updateSpreadsheetWithLocalChanges(
  ...
  for (const sheetTitle of new Set(
    ...
    if (sheetTitle === I18N_SHEET_NAME) {
      console.log(
        `Skipping reserved metadata sheet "${sheetTitle}" – its content is managed separately.`,
      );
      continue;
    }
    ...
    if (!sheet) {
      ...
      console.log(
        `Sheet "${sheetTitle}" not found — creating it with ${localeHeaders.length} locale column(s).`,
      );
    ...
    const rows = await withRetry(() => sheet!.getRows(), `getRows: ${sheetTitle}`, baseDelayMs);
      ...
      for (const key of Object.keys(localeData)) {
        ...
        if (!existingKeys.has(keyLower)) {
          ...
          if (!localeHeader) {
            ...
            localeHeader = originalHeaders.find((h) => getLanguagePrefix(h) === localeLang);
          }
        ...
        for (const [keyLower, rowData] of newKeys.entries()) {
          ...
          if (localesWithValues && localesWithValues.size > 0) {
            ...
            for (const localeHeader of locales) {
              ...
              const exactHeaderName = originalHeaders.find((h) => h.toLowerCase() === localeLower);
