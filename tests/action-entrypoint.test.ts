...
beforeEach(() => {
  ...
  mockManageDriveTranslations.mockResolvedValue({ translations: {} } as Awaited<
    ReturnType<typeof manageDriveTranslations>
  >);
...
describe('action-entrypoint', () => {
    ...
    it('wires asset sync provider and options from action inputs and logs result', async () => {
      ...
      expect(mockInfo).toHaveBeenCalledWith(
        expect.stringContaining(
          'Asset sync completed: 2 manifest entries, 1 downloaded, 0 updated, 0 deleted, 1 skipped.',
        ),
      );
    });
  ...
  describe('outputs', () => {
    it('sets translations-dir as absolute path resolved from workspace', async () => {
      ...
      expect(mockSetOutput).toHaveBeenCalledWith('translations-dir', '/workspace/out/translations');
    });
    ...
    it('sets locales-file as absolute path resolved from workspace', async () => {
      ...
      expect(mockSetOutput).toHaveBeenCalledWith('locales-file', '/workspace/src/i18n/locales.ts');
    });
