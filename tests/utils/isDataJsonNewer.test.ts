...
describe('isDataJsonNewer', () => {
  ...
  test('should return false if data.json does not exist', () => {
    // Mock getFileLastModified to return null (file doesn't exist)
    (getFileLastModified as jest.Mock).mockReturnValue(null);

    const result = isDataJsonNewer('/path/to/data.json', '/path/to/translations');
    
    expect(getFileLastModified).toHaveBeenCalledWith('/path/to/data.json');
    expect(result).toBe(false);
  });

  test('should return true if there are no translation files', () => {
    // Mock getFileLastModified to return a date for data.json
  ...
  test('should return true if data.json is newer than all translation files', () => {
    ...
    (getFileLastModified as jest.Mock).mockImplementation((filePath) => {
      if (filePath === '/path/to/data.json') return dataJsonDate;
  ...
  test('should return false if any translation file is newer than data.json', () => {
    // Mock dates for comparison
    const dataJsonDate = new Date('2023-01-05T12:00:00Z');
    const translationDate1 = new Date('2023-01-01T12:00:00Z');
    const translationDate2 = new Date('2023-01-10T12:00:00Z'); // Newer than data.json
