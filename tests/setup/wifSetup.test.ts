...
function mockSuccessfulSetup(projectNumber = '123456789'): void {
  mockFetch
    ...
    .mockResolvedValueOnce(
      jsonResponse({
        name: 'projects/proj/locations/global/workloadIdentityPools/pool/operations/op1',
        done: true,
      }),
    )
    ...
    .mockResolvedValueOnce(
      jsonResponse({
        name: 'projects/proj/locations/global/workloadIdentityPools/pool/operations/op2',
        done: true,
      }),
    )
    // 4. getIamPolicy
...
describe('setupWIF', () => {
  ...
  describe('idempotency (409 conflict)', () => {
    test('ignores 409 when pool already exists and continues', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(
          jsonResponse({
            name: 'projects/p/locations/global/workloadIdentityPools/p/operations/op',
            done: true,
          }),
        )
    ...
    test('ignores 409 when provider already exists and continues', async () => {
      mockFetch
        .mockResolvedValueOnce(jsonResponse({ projectNumber: '123' }))
        // Pool succeeds
        .mockResolvedValueOnce(jsonResponse({ name: 'p/op', done: true }))
            ...
            bindings: [
              {
                role: 'roles/iam.workloadIdentityUser',
                members: [
                  'principalSet://iam.googleapis.com/projects/123/locations/global/workloadIdentityPools/github-actions/attribute.repository/other/repo',
                ],
              },
            ],
            etag: 'e',
  ...
  describe('error handling', () => {
    test('throws GcpApiError with status for non-409 API errors', async () => {
      mockFetch
        .mockResolvedValueOnce(jsonResponse({ projectNumber: '123' }))
        .mockResolvedValueOnce(jsonResponse({ error: { message: 'Permission denied' } }, 403));
    ...
    test('throws when operation fails with error', async () => {
      mockFetch.mockResolvedValueOnce(jsonResponse({ projectNumber: '123' })).mockResolvedValueOnce(
        jsonResponse({
      ...
      mockFetch
        ...
        .mockResolvedValueOnce(
          jsonResponse({
            ...
            error: { code: 7, message: 'pool failed' },
          }),
        );
      ...
      mockFetch
        ...
        .mockResolvedValueOnce(
          jsonResponse({
            ...
            error: { code: 7, message: 'provider op failed' },
          }),
        );
  ...
  describe('long-running operation polling', () => {
    test('polls until operation is done when initial response is pending', async () => {
      ...
      const pendingOp = {
        name: 'projects/p/locations/global/workloadIdentityPools/pool/operations/op',
        done: false,
      };
      const doneOp = {
        name: 'projects/p/locations/global/workloadIdentityPools/pool/operations/op',
